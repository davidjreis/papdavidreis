...
var core = {
 ...
 = false;
    ...
    containers.status.html('Começar Jogo :)');
    containers.time.html('0');
    containers.msg.html('Clica no quadrado para começar!');
...
var action = {
  ...
  click: function(e){  
    ...
    if(e.which === 1 && globals.flagMap[x][y] !== 1 && defaults.difficulty !== 0){
      ...
      if(globals.firstClick === true){
        ...
        // Set number of mines
        containers.mines.html('Tens que encontrar ' + globals.totalMines + ' minas para ganhar.');
      ...
      if(!util.is('revealed', x, y)){
        ...
        globals.revealedMap[x][y] = 1;
				...
				if(globals.mineMap[x][y] !== -1){
					// 'remove square', by drawing a white one over i
					var alpha = 0.8,
            ...
            // else{
            //   if(globals.mineMap[x][y] ==0){
            //   globals.context.fillStyle = 'none';
  ...
  flag: function(flag, x, y){
    ...
    containers.mines.html('Tens que encontrar ' + (globals.totalMines - globals.totalFlags) + ' minas para ganhar.');
    containers.flags.html('Tu metes-te ' + globals.totalFlags + ' bandeiras.');
  ...
  revealMines: function(mine){
    ...
    globals.fimdejogo
 = true;
    containers.status.html('Perdeu :(');
    containers.msg.html('Tente outra vez');
  ...
  display: function(){
    ...
    if(typeof Storage !== 'undefined'){
      ...
      if(typeof localStorage.scores !== 'undefined'){
        ...
        containers.scoreboard.html('<tr><td>Ainda não ganhastes nenhum jogo :(</td></tr>');
      }
